...
const MyHospitals: React.FC = () => {
  ...
  const exportToCSV = (data: any[]) => {
    ...
    const rows = data.map(item => [item.name, item.address, item.phone].join(",")); // Adjust based on your data structure
    ...
    <section className="bg-Primary py-20 h-auto w-auto">
      ...
      <div className="container overscroll-auto box-border h-auto w-3/4 pb-4  m-auto  rounded-lg  md:w-full">
        <div className="p-2 flex flex-col items-center rounded-md gap-12">
          <form className="w-full md:w-full">
            ...
            <div className="relative">
              ...
              <input
                ...
                placeholder="Search by city or state"ext-sm
                required
                value={searchInput}
          ...
          {isLoading && <p className="text-center text-gray-700">Please wait!!</p>}
          ...
          <div className="flex justify-between items-center gap-4 w-full">
            ...
            <div className="flex gap-4">
              <button className="bg-blue-500 py-2 px-2 rounded-md text-sm font-sans hover:bg-blue-400" onClick={handleShare}>
                Share
              </button>
              <button className="bg-green-500 py-2 px-2 rounded-md text-xs font-sans hover:bg-green-400" onClick={handleExport}>
                Export CSV
              </button>
            </div>
          </div>
          <div className="flex justify-center gap-2 mt-4">
            ...
            {pageNumbers.map(number => (
              number <= totalPages && (
                <button
                  key={number}
                  className={`py-2 px-4 rounded-md text-sm font-sans ${number === currentPage ? 'bg-blue-500 text-white' : 'bg-gray-300 hover:bg-gray-400'}`}
